...
CREATE TABLE IF NOT EXISTS `medications_master` (
    ...
    `item_ingr_name` TEXT NULL COMMENT '주성분명 (ITEM_INGR_NAME 또는 MAIN_ITEM_INGR)',
    ...
    `permit_date` VARCHAR(20) NULL COMMENT '허가일자(단축) (PERMIT_DATE)',
    `permit_no` VARCHAR(100) NULL COMMENT '허가번호 (PERMIT_NO)',
    ...
    `etc_otc_code` VARCHAR(50) NULL COMMENT '전문/일반 구분 코드 (ETC_OTC_CODE)',
    `chart` VARCHAR(500) NULL COMMENT '성상 (CHART)',
    `material_name` TEXT NULL COMMENT '원료성분 (MATERIAL_NAME)',
    `storage_method` VARCHAR(500) NULL COMMENT '저장방법 (STORAGE_METHOD)',
    `valid_term` VARCHAR(200) NULL COMMENT '유효기간 (VALID_TERM)',
    `pack_unit` VARCHAR(500) NULL COMMENT '포장단위 (PACK_UNIT)',
    `raw_json` JSON NULL COMMENT '원본 JSON 문자열',
